...
Sub SetFormColorsInTest()

    robChangeFormColors (False) 'true = set to original
End Sub

Sub robChangeFormColors(original As Boolean)
'https://docs.microsoft.com/en-us/office/vba/api/access.accontroltype
    Dim frm As Form
    Dim formIsLoaded As Boolean
    Dim outputThisProp As Boolean
    Dim changeThisForm As Boolean
    Dim originalColor As Long
    Dim color As Long
    color = RGB(255, 204, 204) ' new pink color
    debuggin = True
    
    For Each frmholder In Application.CurrentProject.AllForms
        'Debug.Print frmholder.Name
        changeThisForm = False
        If frmholder.Name = "frm_Company" Or frmholder.Name = "frm_Invoice" _
            Then
                changeThisForm = True
                originalColor = RGB(192, 192, 192) '#c0c0c0 = 12632256 = this - ORIGINAL color Company and Invoice
        End If
        If frmholder.Name = "Main Switchboard" _
            Or frmholder.Name = "frm_Payroll_Hrs_Points" _
            Then
                changeThisForm = True
                originalColor = -2147483633 'original color for frm_Payroll_hrs_points
        End If
        If changeThisForm = True Then
            formIsLoaded = False
            For Each aLoadedForm In Application.Forms
                If aLoadedForm.Name = frmholder.Name Then
                    formIsLoaded = True
            ...
            If formIsLoaded = False Then
                DoCmd.OpenForm frmholder.Name, acDesign, , , acFormPropertySettings, acHidden
            End If
        
            Set frm = Application.Forms(frmholder.Name)
        
            
            'default backcolor = 12632256
            'Debug.Print "Bsckcolor = " & Trim(frm.Section("Detail").BackColor)
            If original Then
                Debug.Print "Changing " & frm.Name & " from " & frm.Section("Detail").BackColor & " to " & originalColor
                frm.Section("Detail").BackColor = originalColor
            Else
                Debug.Print "Changing " & frm.Name & " from " & frm.Section("Detail").BackColor & " to " & color
                frm.Section("Detail").BackColor = color
            End If
            'ProcessControls frm.controls
